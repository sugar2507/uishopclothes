...
export class SexService {
  ...
  addSex(emp: Sex) {
    return this.http.post<Sex>(this.addSexUrl, emp).pipe(
      map((res: any) => {
        return res;
      })
    );
  ...
  getAllSex() {
    return this.http.get<Sex>(this.getSexUrl).pipe(
      map((res: any) => {
        return res;
      })
    );
  ...
  updateSex(emp: Sex) {
    return this.http.put<Sex>(this.getSexUrl, emp).pipe(
      map((res: any) => {
        return res;
      })
    );
  ...
  getSexById(id: number) {
    return this.http.get<Sex>(this.getSexUrl + '/GetSexById/' + id);
  }
}
